...
async def genv2key(interaction: discord.Interaction, duration: str = "lifetime", amount: int = 1):
    ...
    # Build embed
    embed = discord.Embed(title=f"Vyron V2 Key{'s' if amount > 1 else ''}", color=0x5080FF)
...
@app_commands.describe(user="The user to send the key to", duration="Duration: e.g. 1h, 7d, 2w, 1m, lifetime", amount="Number of keys to generate (1-10)")
async def genv2keyto(interaction: discord.Interaction, user: discord.Member, duration: str = "lifetime", amount: int = 1):
    if not has_owner_role(interaction):
        return await deny(interaction)
    ...
    pub_embed = discord.Embed(
        title=f"🔑 Key{'s' if amount > 1 else ''} Sent",
        description=f"{interaction.user.mention} sent {amount} Vyron V2 key{'s' if amount > 1 else ''} to {user.mention}.",
        color=0x5080FF
    )
    ...
    for key in generated_keys:
        asyncio.create_task(send_webhook(embeds=[{
            ...
            "fields": [
                {"name": "Generated by", "value": f"{interaction.user.mention} ({interaction.user})", "inline": True},
                {"name": "For", "value": f"{user.mention} ({user})", "inline": True},
                {"name": "Duration", "value": duration_label(secs), "inline": True},
                {"name": "Key", "value": f"```{key}```", "inline": False},
            ],
            "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
